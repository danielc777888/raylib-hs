...
loadFont :: String -> IO (Ptr Font)
loadFont f = withCString f (\f' ->
                cLoadFont f')
...
drawText t x y fs c = with c (\c_ptr ->
                                   withCString t (\t' -> cDrawText t' (fromIntegral x) (fromIntegral y) (fromIntegral fs) c_ptr))
...
drawTextEx f_ptr t v fs s c = with c (\c_ptr ->
                                   with v (\v_ptr ->
                                        withCString t (\t' -> cDrawTextEx f_ptr t' v_ptr (realToFrac fs) (realToFrac s) c_ptr)))
